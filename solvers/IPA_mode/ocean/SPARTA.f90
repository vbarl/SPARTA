...
!-------------------------------------------------------------------------------------------------------------------------------!
!
! Found in OceanReflection subroutine
!
complex(8)                  :: CN1,CN2                              ! refractive indices (air, water)
real(8),dimension(1:4,1:4)  :: Refl,Reflec                          ! Reflection matrices
real(8)                     :: SIGMA2, k_z_det                      ! mean square surface slope of the waves, cozine of zeithal angle of detector
!-------------------------------------------------------------------------------------------------------------------------------!
!######################################################### MAIN PROGRAM ########################################################!
!-------------------------------------------------------------------------------------------------------------------------------!
!
...
CN1 = (1d0,0d0)     ! refractive index for air
CN2 = (1.33d0,0d0)  ! refractive index for water
SIGMA2 = 0.00662d0  ! mean square surface slope of the waves
!
!-------------------------------------------------------------------------------------------------------------------------------!
  ...
  call PhotonPath                                     ! Tracke the photon path
   do while (z .lt. cloudtop .and. weight .gt. eps)   ! Trace photon back to cloud top when reflecting surface present
    if (z .gt. cloudbottom) then
        ...
        call OceanReflection                          ! Reflection event
        call PhotonPath                               ! Tracke the photon path
    end if
    end do ! while
...
!-------------------------------------------------------------------------------------------------------------------------------!
!                                                                                                                               !
! SOS: If beta_abs and beta_sca are not available COMMENT OUT lines 486, 487, 514, and 515. BUT, since these variables are in   !
! need, they need to be calculated. Then, go to INIT subroutine and uncomment lines 609 - 616                                   !
!                                                                                                                               !
!-------------------------------------------------------------------------------------------------------------------------------!
!
...
do while (sca_sum .lt. enough)
 ...
 !
 ! IPA t_ray = 1d10
 ! IPA if (abs(k_x_new) .gt. eps) then
 ! IPA  up_or_down_x = int(sign(1.0d0, k_x_new))
 ! IPA  t_ray(1) = ( xmesh(ix + delta_i(up_or_down_x)) - x )/k_x_new
 ! IPA end if
 ! IPA if (abs(k_y_new) .gt. eps) then
 ! IPA  up_or_down_y = int(sign(1.0d0, k_y_new))
 ! IPA  t_ray(2) = ( ymesh(iy + delta_i(up_or_down_y)) - y )/k_y_new
 ! IPA end if
 ! IPA if (abs(k_z_new) .gt. eps) then
  up_or_down_z = int(sign(1.0d0, k_z_new))
  t_ray(3) = ( zmesh(iz + delta_i(up_or_down_z)) - z )/k_z_new
 ! IPA end if

 ! IPA t_ray_min = minval(t_ray)              ! gives the minimum
 ! IPA i_ray_min = minloc(t_ray, dim = 1)     ! gives the place of the minimum

 ! shortest distance is realistic. Intersection with the smallest distance
 ! IPA tau_sum = tau_sum + t_ray_min*local_beta_x
 tau_sum = tau_sum + t_ray(3)*local_beta_x ! IPA 

 ! IPA sca_sum = sca_sum + t_ray_min*local_beta_sca
 ! IPA abs_sum = abs_sum + t_ray_min*local_beta_abs
 sca_sum = sca_sum + t_ray(3)*local_beta_sca ! IPA
 abs_sum = abs_sum + t_ray(3)*local_beta_abs ! IPA


 ! make sure new photon is inside next box
 ! IPA help1 = (t_ray_min + eps)
 help1 = (t_ray(3) + eps) ! IPA
...
! IPA   wasnun:   select case (i_ray_min)

! IPA   case(1)
! IPA    ihelp1 = delta_i(up_or_down_x) ! (-1,+1) -> (0,1)
! IPA    ix = ix + up_or_down_x
! IPA    if (ix .lt. xmin) then
! IPA     ix = xmax; x = cloudright_in
! IPA    end if
! IPA    if (ix .gt. xmax) then
! IPA     ix = xmin; x = cloudleft_in
! IPA    end if

! IPA   case(2)
! IPA    ihelp1 = delta_i(up_or_down_y) ! (-1,+1) -> (0,1)
! IPA    iy = iy + up_or_down_y
! IPA    if (iy .gt. ymax) then
! IPA     iy = ymin; y = cloudfront_in
! IPA    end if
! IPA    if (iy .lt. ymin) then
! IPA     iy = ymax; y = cloudback_in
! IPA    end if
...
! IPA   y = y - step_back*k_y_new
! IPA   x = x - step_back*k_x_new
  !
  !-----------------------------------------------------------------------------------------------------------------------------!
  !                                                                                                                             !
...
!-------------------------------------------------------------------------------------------------------------------------------!
!
! IPA ix = xmin + int( (x - cloudleft)/d_xmesh)
! IPA iy = ymin + int( (y - cloudfront)/d_ymesh)
iz = zmin
zpos = cloudbottom
...
do while (z2detector .lt. cloudtop .and. z2detector .gt. cloudbottom) ! trace out of model domain
 ...
 !
! IPA t_ray = 1d10
! IPA  if (abs(k_x_choice(i_radiance)) .gt. eps) then
! IPA   up_or_down_x = int(sign(1.0d0, k_x_choice(i_radiance)))
! IPA   t_ray(1) = ( xmesh(ix2detector + delta_i(up_or_down_x)) - x2detector )/k_x_choice(i_radiance)
! IPA  end if
! IPA  if (abs(k_y_choice(i_radiance)) .gt. eps) then
! IPA   up_or_down_y = int(sign(1.0d0, k_y_choice(i_radiance)))
! IPA   t_ray(2) = ( ymesh(iy2detector + delta_i(up_or_down_y)) - y2detector )/k_y_choice(i_radiance)
! IPA  end if
! IPA  if (abs(k_z_choice(i_radiance)) .gt. eps) then
...
! IPA  t_ray_min = minval(t_ray)                              ! gives the minimum
! IPA  i_ray_min = minloc(t_ray, dim = 1)                     ! gives the place of the minimum
! IPA  tau_sum = tau_sum + t_ray_min*local_beta_x             ! shortest distance is realistic. Intersection with the smallest distance

 tau_sum = tau_sum + t_ray(3)*local_beta_x   ! IPA


! IPA help1 = (t_ray_min + eps)                              ! make sure new photon is inside next box
 help1 = (t_ray(3) + eps)   ! IPA

! IPA  x2detector = x2detector + help1*k_x_choice(i_radiance)
! IPA  y2detector = y2detector + help1*k_y_choice(i_radiance)
 z2detector = z2detector + help1*k_z_choice(i_radiance)

! IPA  wasnun:   select case (i_ray_min)
! IPA  case(1)
! IPA  ihelp1 = delta_i(up_or_down_x) ! (-1,+1) -> (0,1)
! IPA  ix2detector = ix2detector + up_or_down_x
! IPA  if (ix2detector .lt. xmin) then
! IPA   ix2detector = xmax; x2detector = cloudright_in
! IPA  end if
! IPA  if (ix2detector .gt. xmax) then
! IPA   ix2detector = xmin; x2detector = cloudleft_in
! IPA  end if

! IPA  case(2)
! IPA  ihelp1 = delta_i(up_or_down_y) ! (-1,+1) -> (0,1)
! IPA  iy2detector = iy2detector + up_or_down_y
! IPA  if (iy2detector .gt. ymax) then
! IPA   iy2detector = ymin; y2detector = cloudfront_in
! IPA  end if
! IPA  if (iy2detector .lt. ymin) then
! IPA   iy2detector = ymax; y2detector = cloudback_in
! IPA  end if

! IPA  case(3)
 ihelp1 = delta_i(up_or_down_z) ! (-1,+1) -> (0,1)
 iz2detector = iz2detector + up_or_down_z
! IPA  end select wasnun
end do ! while
!
...
!-------------------------------------------------------------------------------------------------------------------------------!
!
subroutine OceanReflection
!
!-------------------------------------------------------------------------------------------------------------------------------!
!                                                                                                                               !
! To consider anisotropic and polarizing ocean reflection, the code by Mishchenko and Travis (1997) has been implemented into   !
! SPARTA. It calculates the reflection matrix for rough water surfaces utilizing the Fresnel formulas. Same procedure as in     !
...
! the same as when a scattering event takes place
k_x = k_x_new
k_y = k_y_new
k_z = k_z_new
phi_inc = phi_sca
...
! Similar to LocalEstimateMethod: For each reflection event we calculate its contribution to the radiance calculation
do i_radiance = 1, n_radiance
 if (k_z_choice(i_radiance) .gt. 0.0) then
  phi_det = phi_choice(i_radiance)*rad
  k_z_det = k_z_choice(i_radiance)

  ! Calculation of the Stokes reflection matrix (see subroutine RMATR)
  call RMATR (CN1,CN2,SIGMA2,k_z,phi_inc,k_z_det,phi_det,Reflec)

  Reflec = Reflec * k_z_det/pi

  radiance_weight = weight * Reflec(1,1)
  S_out(1) = Reflec(1,1) * S_in(1) + Reflec(1,2) * S_in(2) + Reflec(1,3) * S_in(3) + Reflec(1,4) * S_in(4)
  S_out(2) = Reflec(2,1) * S_in(1) + Reflec(2,2) * S_in(2) + Reflec(2,3) * S_in(3) + Reflec(2,4) * S_in(4)
  S_out(3) = Reflec(3,1) * S_in(1) + Reflec(3,2) * S_in(2) + Reflec(3,3) * S_in(3) + Reflec(3,4) * S_in(4)
  S_out(4) = Reflec(4,1) * S_in(1) + Reflec(4,2) * S_in(2) + Reflec(4,3) * S_in(3) + Reflec(4,4) * S_in(4)

  ! track the photon along its path form the scattering event to the detector position
...
! define the new direction cosines of the reflected radiation
...
! Calculate the Stokes reflection matrix and update the weights
call RMATR (CN1,CN2,SIGMA2,k_z,phi_inc,k_z_new,phi_sca,Refl)

! up date the scalar weight
weight = weight * Refl(1,1)

! update the Stokes weight
S_out(1) = Refl(1,1) * S_in(1) + Refl(1,2) * S_in(2) + Refl(1,3) * S_in(3) + Refl(1,4) * S_in(4)
S_out(2) = Refl(2,1) * S_in(1) + Refl(2,2) * S_in(2) + Refl(2,3) * S_in(3) + Refl(2,4) * S_in(4)
S_out(3) = Refl(3,1) * S_in(1) + Refl(3,2) * S_in(2) + Refl(3,3) * S_in(3) + Refl(3,4) * S_in(4)
S_out(4) = Refl(4,1) * S_in(1) + Refl(4,2) * S_in(2) + Refl(4,3) * S_in(3) + Refl(4,4) * S_in(4)

! reflected Stokes vector is the incident for the next scattering event!
S_in(1) = S_out(1)
S_in(2) = S_out(2)
S_in(3) = S_out(3)
S_in(4) = S_out(4)
!
end subroutine OceanReflection
!
!-------------------------------------------------------------------------------------------------------------------------------!
!###############################################################################################################################!
!###############################################################################################################################!
  ...
  do iy = ymin, ymax                ! loop along y-grid boxes
     ...
     do i_radiance = 1, n_radiance  ! loop along i_radiance
        if (flux_in(ix,iy) > 0d0) then
           ...
           !                                                                                                                !
           ! Bottom line, uncomment lines 1935 and 1936 and comment out 1940 and 1941                                       !
           !                                                                                                                !
  ...
  do iy = ymin, ymax
    do i = 1, n_radiance
      ...
      if (flux_S(ix,iy) > 0d0) then
        ...
        ! The normalized radiance is not given by equation 3.3.8 (Barlakas, 2016b), but by equation 4.4 (Barlakas, 2016b)!
        !                                                                                                                !
        ! Bottom line, uncomment lines 1981 - 1991 and comment out 1996 - 2006                                           !
        !                                                                                                                !
...
! IPA open(10, file = 'output/'//trim(mainfile)//'.H')
! IPA   do ix = xmin, xmax
! IPA     do iy = ymin, ymax
! IPA       write(10, '(f8.5)') net_horizontal_transport(ix,iy) ! I3RC convention
! IPA   end do
! IPA end do
! IPA close(10)
...
!-------------------------------------------------------------------------------------------------------------------------------!
!                                                                                                                               !
! Calculation of the Stokes reflection matrix for illumination from above a statistically rough surface seperating two half-    !
! spaces with refractive indices of the upper and lower half-spaces equal to CN1 and CN2, respectively. The effect of shadowing !
! is included. For all formulas, see the paper Mishchenko and Travis (1997)                                                     !
!                                                                                                                               !
!-------------------------------------------------------------------------------------------------------------------------------!
!
! Declare variables
...
complex(8) :: CN1, CN2, CXI2, C1, C2, CRPER, CRPAR
complex(8) :: CF11, CF12, CF21, CF22
complex(8) :: CI, C21, C22, CTTTP, CTTPT, CTTPP, CTPPT, CTPPP, CPTPP
real(8)    :: R(4,4),phii,PHIR
!
! Input information
!
! SIGMA2 = mean square surface slope
...
!   cartesian components of the unit vectors of the incident and scattered beams
DMUI = dabs(DMUI)
DMUR = dabs(DMUR)
